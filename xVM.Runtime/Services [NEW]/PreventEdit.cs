...
namespace xVM.Runtime.Services
{
    internal class PreventEdit<T>
    {
        ...
        [VMProtect.BeginMutation]
        public PreventEdit(T val, int salt)
        {
            _obf = Obfuscate(val, salt);
        }

        //allow normal read/write
        public static implicit operator T(PreventEdit<T> value) => Deobfuscate(value._obf, value._salt);
        public static implicit operator PreventEdit<T>(T value) => new PreventEdit<T>(value);
