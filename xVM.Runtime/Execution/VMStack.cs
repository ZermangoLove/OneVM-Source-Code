...
namespace xVM.Runtime.Execution
{
    ...
    {
        private static readonly int SectionSize = 6; // 1 << 6 = 64
        private static readonly int IndexMask = (1 << SectionSize) - 1;
        ...
        public unsafe IntPtr Localloc(uint guardPos, uint size)
        {
            var node = new LocallocNode
            {
                GuardPos = guardPos,
                Memory = (IntPtr)NativeMethods.malloc(size)
            };
        ...
        private class LocallocNode
        {
            ...
            ~LocallocNode()
            {
                if(Memory != NativeMethods.NULL)
                {
                    NativeMethods.FreeMemory(Memory);
                    Memory = IntPtr.Zero;
                }
            }
