...
namespace xVM.Helper.Core.Helpers {
	...
	/// </summary>
	internal static class MutationHelperCore
    {
        ...
        internal static void InjectKey_Int(MethodDef method, int keyId, int key)
        {
            foreach (Instruction instr in method.Body.Instructions)
            {
                if (instr.OpCode == OpCodes.Ldsfld)
                {
                    ...
                    int _keyId;
                    if (field.DeclaringType.FullName == RTMap.MutationCore && Field2IntIndex.TryGetValue(field.Name, out _keyId) && _keyId == keyId)
                    {
        ...
        internal static void InjectKey_Long(MethodDef method, int keyId, long key) {
			foreach (Instruction instr in method.Body.Instructions)
            {
                if (instr.OpCode == OpCodes.Ldsfld)
                {
                    var field = instr.Operand as IField;
                    int _keyId;
                    if (field.DeclaringType.FullName == RTMap.MutationCore && Field2LongIndex.TryGetValue(field.Name, out _keyId) && _keyId == keyId)
                    {
        ...
        internal static void InjectKey_String(MethodDef method, int keyId, string key)
        {
            foreach (Instruction instr in method.Body.Instructions)
            {
                if (instr.OpCode == OpCodes.Ldsfld)
                {
                    var field = instr.Operand as IField;
                    string _keyId;
                    if (field.DeclaringType.FullName == RTMap.MutationCore && Field2LdstrIndex.TryGetValue(field.Name, out _keyId) && _keyId == keyId.ToString())
                    {
        ...
        internal static void InjectKeys_Int(MethodDef method, int[] keyIds, int[] keys)
        {
            foreach (Instruction instr in method.Body.Instructions)
            {
                if (instr.OpCode == OpCodes.Ldsfld)
                {
                    var field = instr.Operand as IField;
                    int _keyIndex;
                    if (field.DeclaringType.FullName == RTMap.MutationCore &&
        ...
        internal static void ReplaceValue_T(MethodDef method, Instruction ret_inst)
        {
            for (int i = 0; i < method.Body.Instructions.Count; i++)
            {
                ...
                if (instr.OpCode == OpCodes.Call &&
                    md.DeclaringType.Name == RTMap.MutationCore &&
                    md.Name == RTMap.MutationCore_Value_T)
                {
        ...
        internal static void ReplacePlaceholder(MethodDef method, Func<Instruction[], Instruction[]> repl) {
			...
			for (int i = 0; i < method.Body.Instructions.Count; i++) {
				...
				if (instr.OpCode == OpCodes.Call) {
					...
					if (operand.DeclaringType.FullName == RTMap.MutationCore &&
					    operand.Name == RTMap.MutationCore_Placeholder) {
