...
{
    internal class ConstantsProtection
    {
        ...
        public void Execute(ModuleDef module, VMRuntime runtime)
        {
            ...
            context.ReferenceRepl = new Dictionary<MethodDef, List<Tuple<Instruction, uint, IMethod>>>();
            context.EncodedBuffer = new List<uint>();
        }

        private void InjectHelpers(CEContext moduleCtx)
        {
            moduleCtx.InitMethod = context.VMRuntime.RTSearch.ConstantsProt_Initialize;
            moduleCtx.Decoders = new List<Tuple<MethodDef, DecoderDesc>>();

            for (int i = 0; i < moduleCtx.DecoderCount; i++)
            {
                MethodDef decoderInst = context.VMRuntime.RTSearch.ConstantsProt_Get;
                ...
                // Constants.Initialize Method Add Constants Static Ctor
                //MethodDef cctor = context.VMRuntime.RTSearch.ConstantsProt.FindOrCreateStaticConstructor();
                //cctor.Body.Instructions.Insert(0, Instruction.Create(OpCodes.Call, context.Module.Import(context.InitMethod)));
                ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            }
        }
