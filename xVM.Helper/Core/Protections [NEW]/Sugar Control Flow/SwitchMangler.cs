...
namespace xVM.Helper.Core.Protections.SugarControlFlow
{
    internal class SwitchMangler : ManglerBase
    {
        ...
        public override void Mangle(CilBody body, SugarScopeBlock root, MethodDef Method, TypeSig retType)
        {
            ...
            body.MaxStack += 2;
            IPredicate predicate = new Predicate(Method.Module);

            foreach (SugarInstrBlock block in GetAllBlocks(root))
            {
        ...
        LinkedList<Instruction[]> SplitStatements(SugarInstrBlock block, Trace trace)
        {
        ...
        {
            if (((!Method.Module.IsClr40 && false) && (!Method.DeclaringType.HasGenericParameters && !Method.HasGenericParameters)) && ((instrs[0].OpCode.FlowControl == FlowControl.Call) || (instrs[0].OpCode.FlowControl == FlowControl.Next)))
            {
                    ...
                    case 2:
                        {
                            ...
                            {
                                TypeDef def = Method.Module.Types[Random.NextInt32(Method.Module.Types.Count)];
                                if (def.HasMethods)
                                {
                                    instrs.Add(Instruction.Create(OpCodes.Ldtoken, def.Methods[Random.NextInt32(def.Methods.Count)]));
                                    instrs.Add(Instruction.Create(OpCodes.Box, (ITypeDefOrRef)Method.Module.CorLibTypes.GetTypeRef("System", "RuntimeMethodHandle")));
                                    flag = true;
                                }
                            }
                            if (!flag)
        ...
        {
            if (!Method.Module.IsClr40 && false)
            {
                switch (new RandomGenerator().NextInt32(6))
                {
                    case 0:
                    ...
                    case 5:
                        instrs.Add(Instruction.Create(OpCodes.Ldtoken, (IMethod)Method));
                        return;
                }
            }
        }
