...
{
    internal static class SugarBlockParser
    {
        public static SugarScopeBlock ParseBody(CilBody body)
        {
            var ehScopes = new Dictionary<ExceptionHandler, Tuple<SugarScopeBlock, SugarScopeBlock, SugarScopeBlock>>();

            foreach (ExceptionHandler eh in body.ExceptionHandlers)
            {
                var tryBlock = new SugarScopeBlock(BlockType.Try, eh);
                ...
                var handlerBlock = new SugarScopeBlock(handlerType, eh);

                if (eh.FilterStart != null)
                {
                    var filterBlock = new SugarScopeBlock(BlockType.Filter, eh);
                    ehScopes[eh] = Tuple.Create(tryBlock, handlerBlock, filterBlock);
                }
                else
                    ehScopes[eh] = Tuple.Create(tryBlock, handlerBlock, (SugarScopeBlock)null);
            }

            var root = new SugarScopeBlock(BlockType.Normal, null);
            var scopeStack = new Stack<SugarScopeBlock>();
            ...
            foreach (Instruction instr in body.Instructions)
            {
                foreach (ExceptionHandler eh in body.ExceptionHandlers)
                {
                    Tuple<SugarScopeBlock, SugarScopeBlock, SugarScopeBlock> ehScope = ehScopes[eh];
                ...
                {
                    Tuple<SugarScopeBlock, SugarScopeBlock, SugarScopeBlock> ehScope = ehScopes[eh];
                    SugarScopeBlock parent = scopeStack.Count > 0 ? scopeStack.Peek() : null;
                ...
                SugarScopeBlock scope = scopeStack.Peek();

                var block = scope.Children.LastOrDefault() as SugarInstrBlock;

                if (block == null)
                    scope.Children.Add(block = new SugarInstrBlock());
        ...
        internal class SugarScopeBlock : BlockBase
        {
            public SugarScopeBlock(BlockType type, ExceptionHandler handler) : base(type)
            {
                Handler = handler;
            ...
            public Instruction GetFirstInstr()
            {
                ...
                if (firstBlock is SugarScopeBlock)
                    return ((SugarScopeBlock)firstBlock).GetFirstInstr();
                return ((SugarInstrBlock)firstBlock).Instructions.First();
            }
            ...
            public Instruction GetLastInstr()
            {
                ...
                if (firstBlock is SugarScopeBlock)
                    return ((SugarScopeBlock)firstBlock).GetLastInstr();
                return ((SugarInstrBlock)firstBlock).Instructions.Last();
            }
        ...
        internal class SugarInstrBlock : BlockBase
        {
            public SugarInstrBlock() : base(BlockType.Normal)
            {
