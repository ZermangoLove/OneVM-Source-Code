...
namespace xVM.Helper.Core.Protections.Mutation.MethodBlocks
{
    ...
    */
    internal static class SugarMTBlockParser
    {
    ...
    public class ControlFlowGraph : IEnumerable<ControlFlowBlock>
    {
        ...
        readonly int[] SugarInstrBlocks;
        readonly Dictionary<Instruction, int> indexMap;

        ControlFlowGraph(CilBody body)
        {
            this.body = body;
            SugarInstrBlocks = new int[body.Instructions.Count];
        ...
        {
            return blocks[SugarInstrBlocks[instrIndex]];
        }
        ...
        void SplitBlocks(HashSet<Instruction> blockHeaders, HashSet<Instruction> entryHeaders)
        {
            ...
            for (int i = 0; i < body.Instructions.Count; i++)
            {
                ...
                SugarInstrBlocks[i] = currentBlockId;
            }
        ...
        void LinkBlocks()
        {
            for (int i = 0; i < body.Instructions.Count; i++)
            {
                ...
                if (instr.Operand is Instruction)
                {
                    ControlFlowBlock srcBlock = blocks[SugarInstrBlocks[i]];
                    ControlFlowBlock dstBlock = blocks[SugarInstrBlocks[indexMap[(Instruction)instr.Operand]]];
                ...
                else if (instr.Operand is Instruction[])
                {
                    ...
                    {
                        ControlFlowBlock srcBlock = blocks[SugarInstrBlocks[i]];
                        ControlFlowBlock dstBlock = blocks[SugarInstrBlocks[indexMap[target]]];
                        dstBlock.Sources.Add(srcBlock);
                        srcBlock.Targets.Add(dstBlock);
                    }
                }
