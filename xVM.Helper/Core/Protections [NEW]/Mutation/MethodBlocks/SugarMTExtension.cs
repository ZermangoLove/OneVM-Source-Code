...
using System.Linq;
using static xVM.Helper.Core.Protections.Mutation.MethodBlocks.SugarMTBlockParser;

namespace xVM.Helper.Core.Protections.Mutation.MethodBlocks
{
    public static class SugarMTExtension
    {
        public static List<SugarMTBlock> GetBlocks(this MethodDef method)
        {
            List<SugarMTBlock> blocks = new List<SugarMTBlock>();
            ...
            foreach (SugarInstrBlock block in GetAllBlocks(root))
            {
                ...
                foreach (var statement in statements)
                {
                    ...
                    blocks.Add(new SugarMTBlock(statement, isInException, !hasUnknownSource(statement), isInBranch));
                }
            }
        ...
        static LinkedList<Instruction[]> SplitStatements(SugarInstrBlock block, Trace trace)
        {
        ...
        static IEnumerable<SugarInstrBlock> GetAllBlocks(ScopeBlock scope)
        {
            foreach (BlockBase child in scope.Children)
            {
                ...
                {
                    foreach (SugarInstrBlock block in GetAllBlocks((ScopeBlock)child))
                        yield return block;
                }
            }
        }
