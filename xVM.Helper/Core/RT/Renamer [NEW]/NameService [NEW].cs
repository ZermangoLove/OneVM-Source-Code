...
namespace xVM.Helper.Core.RT.Renamer
{
    internal class NameService
    {
        ...
        public void Process(VMRuntime runtime)
        {
            try
            {
                foreach (var type in runtime.RTModule.GetTypes())
                {
                    ...
                    {
                        if (type.Name != "HandleProcessCorruptedStateExceptionsAttribute")
                        {
                            type.Namespace = string.Empty;
                            type.Name = NewName(type.Name);
                        }
                            ...
                            if (method.HasBody)
                                foreach (var instr in method.Body.Instructions)
                                {
                                    ...
                                    if (memberRef != null)
                                    {
                                        ...
                                        if (typeDef != null && typeDef.Module == runtime.RTModule)
