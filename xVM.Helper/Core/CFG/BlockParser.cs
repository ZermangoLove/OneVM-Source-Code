...
namespace xVM.Helper.Core.CFG
{
    public class BlockParser
    {
        ...
        private static HashSet<BasicBlock<CILInstrList>> SplitBlocks(CilBody body, HashSet<Instruction> headers,
            HashSet<Instruction> entries)
        {
            var nextBlockId = 0;
            var currentBlockId = -1;
            Instruction currentBlockHdr = null;
            var blocks = new HashSet<BasicBlock<CILInstrList>>();
            ...
            if(blocks.Count == 0 || blocks.ToList()[blocks.Count - 1].Id != currentBlockId)
            {
        ...
        private static void LinkBlocks(HashSet<BasicBlock<CILInstrList>> blocks)
        {
            var instrMap = blocks
            ...
            {
                var footer = blocks.ToList()[i].Content.Last();
                if(footer.OpCode.FlowControl != FlowControl.Branch &&
                   ...
                   i + 1 < blocks.Count)
                {
                    var src = blocks.ToList()[i];
                    var dst = blocks.ToList()[i + 1];
        ...
        private static ScopeBlock AssignScopes(CilBody body, HashSet<BasicBlock<CILInstrList>> blocks)
        {
